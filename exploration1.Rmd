...
**Thoughts: Unfair comparisons --> adjustments**
...
```{r}
boxplot(hlphrs~postbomb,data = wrkdat)
require(ggplot2)
ggplot(data = wrkdat, aes(x=postbomb, y=hlphrs)) + 
geom_boxplot(aes(fill=GOR))
p + facet_wrap( ~ hlphrs)
...
**They threw out people with age 0, clearly invalid. Are there other categories that have weird or wrong data in them? Should we toss them or do something else with them? Do we need compliance standards for keeping data?**
...
** Maybe put in a box plot**
**address with readings -- 1.2.1 well-defined treatment, well-defined time, clear distinction between covariates -- 1.2.2 treatment seems haphazard, not obviously related to the treatment adn control outcomes -- 1.2.3 treated and control groups comparable: table demonstrating that matching was effective in balancing ovserved covariates. No way to anticipate that unmeasured covariates are similartly balanced -- 1.2.4 eliminate plausible alternatives: use other data collected to see if groups affected by different biases. Also, collect that data.-- 1.2.5 exclusion criteria: inclusion or exclusion based on variables prior to treatment assignment. So probably wouldn't jsut toss out "no age?" -- 1.2.6 leaving a group - noncompliance? -- 1.2.7 study protocol - do not know. what was this survey even for? **
...
**Thoughts: common support/extrapolation problem?  parallel lines requirement? **
